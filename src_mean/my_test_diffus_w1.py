...
# -------------------------------
num_ensemble=50
best_model_path = f"/.../Model_dif/ensmean_Test_5/monthday_{month}/best_model.pt"
ifs_dir = '/.../mpydownscaling/DATA/'
obs_dir = '/.../mpydownscaling/DATA/'
mask_path = '/store_new/mch/msclim/mpydownscaling/DATA/TabsD_mask_static.nc'
results_dir = f"/.../Results_dif/ensmean_Test_5/monthday_{month}/test_predictions"
...
for batch in tqdm(dataloader_test, desc="Inference"):
    ...
    for n in range(num_ensemble):
        print('THIS IS THE NUMBER OF ENS MEMBER: ',n)
...
# Save results
# -------------------------------
save_path = os.path.join(results_dir, "predictions_test5_50members.npz")
np.savez_compressed(
    save_path,
    coarse=all_coarse,
    fine=all_fine,
