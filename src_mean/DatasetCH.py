...
class UpscaleDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, coarse_data_dir, highres_data_dir, month, year_start, year_end,
        ...
        # TabsD data instead of nan have zeros. A static mask is optionally used for training.
        self.highres_filenames = [f"TabsD_expanded_celc_EXT_HC_{year}0{month}_T_2M.nc" for year in range(year_start, year_end)]
        ...
        # Dimensions
        self.ensemble_dim = 1 # changed this due to ens_mean dim #ds_coarse.dims.get("epsd_1", 1)
        ...
        print(f"Loaded coarse data shape before ens_mean: {ds_all_coarse['T_2M'].shape}")
        ...
        self.data_coarse = torch.from_numpy(((ds_all_coarse["T_2M"]).mean(dim='epsd_1', keepdims=True)).values).float()
        print(f"Loaded coarse data ens_mean shape: {((ds_all_coarse['T_2M']).mean(dim='epsd_1', keepdims=True)).shape}")
        ...
        self.residual_std = residual.std()
        self.targets = (residual - self.residual_mean) / self.residual_std
        ...
        if self.constant_variables is not None and constant_variables_filename is not None:
            ...
            # Create lead time index labels [0...45] for each forecast step
            # Assuming 46 lead times
            n_leadtimes = 46
            lead_time_idx = torch.arange(n_leadtimes).repeat(len(self.coarse) // n_leadtimes)
            self.lead_time_idx = lead_time_idx.unsqueeze(1).float()  # Shape: [N, 1]
    ...
    def __getitem__(self, idx):
        lead_time = idx % 46  # or however many time steps you have per year
        lead_time_normalized = torch.tensor([lead_time / 45.0], dtype=torch.float32)
    ...
    def residual_to_fine_image(self, residual, coarse):
        ...
        # Denormalize the predicted residual
        denormalized_residual = (residual * self.residual_std) + self.residual_mean

        # Add the denormalized residual to the coarse input
        return denormalized_residual + coarse
