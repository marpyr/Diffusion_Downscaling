...
# Make dirs
mdir=f"/.../Model_dif/ensmean_Test_4/monthday_{month}"
rdir=f"/.../Results_dif/ensmean_Test_4/monthday_{month}"
...
for step in range(resume_from_epoch, num_epochs):
    # The files saved here are huge, u can comment them out like in w1 script
    # model_save
    ...
    # Save full state (last checkpoint) so that we resume training from here if needed
    checkpoint = {
        "epoch": step,
