...
class Conv2d(torch.nn.Module):
    ...
    def forward_old(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            ...
            if w is not None:
                x = torch.nn.functional.conv2d(x, w, padding=w_pad, groups=self.in_channels if self.in_channels > 1 else 1)
        if b is not None:
    ...
    def forward(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            ...
            if self.down:
                x = torch.nn.functional.conv2d(x, f.tile([self.in_channels, 1, 1, 1]), groups=self.in_channels, stride=2, padding=f_pad)
            if w is not None:
                x = torch.nn.functional.conv2d(x, w, padding=w_pad, groups=1)  # Force groups=1 for standard convolution
        if b is not None:
            x = x.add_(b.reshape(1, -1, 1, 1))
        return x
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        #print(f"conv0 in_channels: {self.conv0.in_channels}, groups: {self.conv0.groups}") # Added this, but no groups here
    ...
    def forward(self, x, emb):
        orig = x
        print(f"Input shape to conv0: {x.shape}")  # Added this line
...
class UNet(torch.nn.Module):
    def __init__(self,
        ...
        model_channels      = 128,          # Base multiplier for the number of
                 # channels.
        channel_mult        = [1,2,4,8],    # Per-resolution multipliers for the number of channels.
    ...
    def forward(self, x, noise_labels=None, class_labels=None,
        ...
        for block in self.dec.values():
            if x.shape[1] != block.in_channels:
                print(f"x.shape: {x.shape}, skips[-1].shape: {skips[-1].shape}")  # Added this line
                x = torch.cat([x, skips.pop()], dim=1)
            x = block(x, emb)
        x = self.out_conv(silu(self.out_norm(x)))
        return x

    def forward_shape_cuts(self, x, noise_labels=None, class_labels=None, augment_labels=None):
        # Mapping.
        emb = torch.zeros([1, self.map_layer1.in_features], device=x.device)
        if self.map_label is not None:
            tmp = class_labels
        ...
        for block in self.dec.values():
            if x.shape[1] != block.in_channels:
                skip = skips.pop()
                # --- Crop skip tensor if spatial dimensions do not match ---
                if skip.shape[2:] != x.shape[2:]:
                    diffY = skip.size(2) - x.size(2)
                    diffX = skip.size(3) - x.size(3)
                    # Crop along height (Y) and width (X)
